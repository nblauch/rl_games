...
class BaseModelNetwork(nn.Module):
    def __init__(self, obs_shape, normalize_value, normalize_input, value_size, normalize_keys=None):
        ...
        if normalize_value:
            self.value_mean_std = torch.jit.script(RunningMeanStd((self.value_size,), name='value_mean_std'))
        if normalize_input:
            if isinstance(obs_shape, dict):
                self.running_mean_std = torch.jit.script(RunningMeanStdObs(obs_shape), normalize_keys=normalize_keys)
            else:
                self.running_mean_std = torch.jit.script(RunningMeanStd(obs_shape), name='running_mean_std')
